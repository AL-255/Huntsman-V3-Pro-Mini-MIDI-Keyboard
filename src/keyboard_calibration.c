...
static bool layout_ok(uint8_t p, uint8_t n) { return p >= 1u && p <= 3u && n == (p == 3u ? 65u : 60u+p); }
...
void calibration_tick(keyboard_calibration_t *s, bool healthy, uint32_t now)
{
    ...
    else if ((uint32_t)(now-s->activity) >= 5000u) calibration_abort(s,CAL_TIMEOUT,now);
}
bool calibration_bounds_valid(uint8_t profile, uint8_t count, const uint16_t *lo, const uint16_t *hi)
{
    ...
    for (unsigned i=0; i<count; ++i)
        if (!lo[i] || hi[i] > 4096u || hi[i] < lo[i]+512u) return false;
    return true;
}
void calibration_frame(keyboard_calibration_t *s, const uint16_t *raw, bool valid, bool neutral, uint32_t now)
{
    ...
    if (s->state == CAL_RELEASE || s->state == CAL_SETTLE) {
        ...
        if ((uint32_t)(now-s->since)<500u) return;
        /* First whole valid frame after 500ms of all keys released. */
        for (unsigned i=0; i<s->count; ++i) {
            if (raw[i]<2048u) { calibration_abort(s,CAL_INVALID,now); return; }
            s->upper[i]=raw[i];
        }
    ...
    for (unsigned key=0; key<s->count; ++key) {
        ...
        if (raw[key]>s->upper[key]/2u) { h->active=false; h->samples=0; continue; }
        if (!h->active) {
...
void calibration_lights(const keyboard_calibration_t *s, uint8_t *rgb, uint32_t now)
{
    ...
    if (!calibration_active(s) && (uint32_t)(now-s->since)>=1500u) return;
    for (unsigned i=0; i<s->count; ++i) {
        unsigned r=0,g=0,b=48;
        if (done(s,i)) { g=96; b=0; }
        if (s->state == CAL_RELEASE || s->state == CAL_SETTLE) { r=32; b=64; }
        if (s->state == CAL_COLLECT && s->holds[i].active) { r=128; g=48; b=0; }
        if (s->state == CAL_ABORTED || s->state == CAL_ERROR) { r=96; g=b=0; }
        const lighting_channels_t *c=&g_lighting_channels[s->profile-1u][i];
        uint8_t *p=rgb+(unsigned)c->controller*192u;
