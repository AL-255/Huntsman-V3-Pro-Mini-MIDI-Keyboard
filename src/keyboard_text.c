...
void keyboard_text_start(keyboard_text_t *s, uint8_t profile, const char *text, uint32_t now)
{
    ...
    if (!text || profile < 1 || profile > 3) return;
    uint8_t letters[29];
    memset(letters, 255, sizeof(letters));
    const unsigned count = profile == 3 ? 65 : 60 + profile;
...
{
    const lighting_channels_t *ch = &g_lighting_channels[s->profile - 1][s->sensors[position]];
    uint8_t *p = frame + ch->controller * 192u;
    p[ch->red] = (s->color[0]*pwm+127u)/255u;
    p[ch->green] = (s->color[1]*pwm+127u)/255u;
    p[ch->blue] = (s->color[2]*pwm+127u)/255u;
}
